# Valid-Distribution N-gram + Bayesian First-Match + Pre-Enrichment + XSA

> **Status: Active development** — Score-first TTT integration in progress.

val_bpb: **0.3922** (valid 1024-token normalized n-gram, Bayesian first-match, fixed 0.5 blend)
| 1.1478 (sliding window) | 14.94 MB | 8xH100 SXM, EVAL_ONLY

| Metric | PR #810 (collision) | This PR (valid) |
|---|---|---|
| val_bpb | 0.2722 | **0.3922** |
| Sliding BPB | 1.1478 | 1.1478 |
...
| Eval method | sliding | sliding | sliding | sliding | 5-gram | 2-7 backoff | 2-11 backoff | +PE conf | shared cache | +phrase cache | **valid distributions** |

v11 is intentionally higher than v10. I replaced collision-exploiting scoring with
properly normalized 1024-token distributions. The 0.12 BPP increase is the **collision
premium** — the portion of n-gram gain that comes from inflated pseudo-probabilities
...
- This submission (valid 1024-token normalization): 0.3922 vs 0.2722 = **0.120 BPP collision premium**
- The remaining 0.756 BPP gain (1.1478 → 0.3922) is genuine n-gram signal


## Key Contributions

### Valid 1024-Token Distribution Scoring (novel, eval-only)
...
p_ng = pair_counts / pair_counts.sum(dim=1, keepdim=True)               # valid distribution
```
This is the only submission computing n-gram probabilities across ALL tokens.
...
The progression proves: once distributions are valid, simple mixing outperforms
sophisticated approaches. The n-gram signal is real but sparse — trust it or don't,
but don't modulate it with adaptive schemes that attenuate the weak signal further.

### Phrase Cache Removed
Dropped entirely. The phrase cache uses the same hash-table structure and suffers
the same collision inflation. For a valid-distribution submission, it cannot be
included without the same 1024-way normalization treatment.

## Neural Architecture (unchanged from PR #810)

- **Model**: 10L, 512d, 8H/4KV GQA, MLP 3x, tied embeddings, U-Net skip connections
- **GELU Pre-Enrichment** (512->768->512): wider nonlinear transformation before transformer blocks (original)
...
- **EMA** (decay=0.997) on GPU: 37% faster training (64.7ms vs 101ms/step) (original)
...
- **Valid distributions**: all n-gram probabilities sum to 1.0 across vocabulary

## Reproduction

Same artifact as PR #810. EVAL_ONLY mode:
```
python3 data/cached_challenge_fineweb.py --variant sp1024 --train-shards 1
EVAL_ONLY=1 torchrun --standalone --nproc_per_node=8 train_gpt.py
```
8xH100 SXM, ~193s eval time.
...
- Valid 1024-token distribution scoring — original to this submission
...
- **v11 (0.3922): valid 1024-token distribution scoring + Bayesian first-match + collision premium analysis**
