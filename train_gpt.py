...
class Hyperparameters:
    ...
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 3500 if _RUN_CONFIG == "A" else 2600))
    warmup_steps = int(os.environ.get("WARMUP_STEPS", 20))
    train_batch_tokens = int(os.environ.get("TRAIN_BATCH_TOKENS", 524_288))
    train_seq_len = int(os.environ.get("TRAIN_SEQ_LEN", 2048 if _RUN_CONFIG == "A" else 1024))
...
def eval_val_sliding(
    ...
    ttt_optimizer = torch.optim.AdamW(ttt_params, lr=ttt_lr, weight_decay=0.0)
    ...
    for win_start, score_start in my_windows:
        mi = min((win_start + seq_len) // ttt_chunk_tokens, num_mega - 1)
    ...
    for mi in range(num_mega):
        ...
        scored_pos, scored_tgt, scored_mp, scored_H, scored_pe = [], [], [], [], []
        for bi in range(0, len(mw), batch_size):
            ...
            for idx, (ws, ss) in enumerate(bw):
                ...
                scored_H.append(ent[idx, ss:]); scored_pe.append(pe_delta[idx, ss:])
        if not scored_pos: continue
        ap = torch.cat(scored_pos); at = torch.cat(scored_tgt); amp = torch.cat(scored_mp)
        aH = torch.cat(scored_H); aPE = torch.cat(scored_pe)
        if distributed:
            _all = [ap, at, amp, aH, aPE]
            for ti in range(len(_all)):
            ...
            ap, at, amp, aH, aPE = _all
        si = ap.argsort(); ap = ap[si]; at = at[si]; amp = amp[si]; aH = aH[si]; aPE = aPE[si]
        n = ap.shape[0]
        ...
        if ttt_epochs > 0 and model is not None:
            ...
            cs = mi * ttt_chunk_tokens; ce = min(cs + ttt_chunk_tokens + 1, total_tokens)
            chunk_toks = vt_gpu[cs:ce]
            for block in base_model.blocks:
                block.attn.rotary._cos_cached = None; block.attn.rotary._sin_cached = None
            base_model.train()
            for _ in range(ttt_epochs):
                ns = (chunk_toks.numel() - 1) // ttt_seq_len
                if ns == 0: break
                tx = chunk_toks[:ns * ttt_seq_len].reshape(ns, ttt_seq_len)
                ty = chunk_toks[1:ns * ttt_seq_len + 1].reshape(ns, ttt_seq_len)
                ttt_optimizer.zero_grad(set_to_none=True)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    ttt_loss = base_model(tx, ty)
                ttt_loss.backward()
                ttt_optimizer.step()
                ttt_step_count += 1
            with torch.no_grad():
                for nm, p in base_model.named_parameters():
                    ema_weights[nm].mul_(ttt_ema_decay).add_(p.data, alpha=1 - ttt_ema_decay)
